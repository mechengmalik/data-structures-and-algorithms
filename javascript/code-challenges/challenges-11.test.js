...
function transformToLis(obj){
  ...
  key.map((item)=>{
    let reFactor = '<li>' + item[0]+ ': ' + item[1]+ '</li>';
    list.push(reFactor);
  });
  return list;
}
...
const totalSum = (input) => {
  // Solution code here...
  return input.map((num1)=>{
    return num1.reduce((acc,value)=>{
      return acc + value;
    });
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  return input.map((item)=>{
    ...
    .map((item2)=>{
      return item2.map((num2)=>{
        if (num2 !== undefined){
          return(Math.pow(2, num2));
        }
...
let findShortest = (data) => {
  ...
  return data.reduce((acc,val)=>{
    return Number (acc.height)-Number(val.height)<0 ?acc:val;
  },0).name;
